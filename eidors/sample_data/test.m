...
C = fscanf(fileID,formatSpec,[928 100]);
...
%========================Decrease Noise======================================
 fmdl = mdl_normalize(mk_library_model('neonate_32el'),1);
 fmdl = mdl_normalize(mk_library_model('adult_male_32el'),1);
 elec_pos = [32,1,.5]; elec_shape=[0.15,0.3,0.01,0,60]; maxsz=0.08; nfft=27;
 fmdl = mk_library_model({'neonate','boundary','left_lung','right_lung'}, ...
       elec_pos, elec_shape, maxsz,nfft);
       
 fmdl = mdl_normalize(fmdl,1);
 fmdl = mdl_normalize(mk_library_model('cylinder_32x1el_coarse'),1);
 options = {'no_meas_current','no_rotate_meas'};
 [fmdl.stimulation,fmdl.meas_select] = mk_stim_patterns(32,1,'{ad}','{ad}',options, 5);
